...
class AddNorm(nn.Module):
    ...
    def forward(self, X, Y):
        Y_hat=self.dropout(Y)
        result=self.ln(Y_hat+X)
        return result
...
class BERTEncoder(nn.Module):
    ...
    def forward(self, tokens, valid_lens):
        ...
        def attention_blocks(X):
            for blk in self.blks:
                X = blk(X, valid_lens)
                return X
        X=checkpoint(attention_blocks, X)
        return X
        '''
        for blk in self.blks:
            X=checkpoint(blk, X, valid_lens)
        return X
        '''
...
class BERTModel(nn.Module):
    ...
    def forward(self, tokens, valid_lens=None,
        ...
        if pred_positions is not None:
            mlm_Y_hat = checkpoint(self.mlm, encoded_X, pred_positions)
        else:
            mlm_Y_hat = None
